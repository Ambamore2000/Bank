...
#include <string>
#include <vector>
// REMOVED FOR LIUNX SUPPORT #include <Windows.h>
...
const void printConsole(const string output) {
	// REMOVED FOR LIUNX SUPPORT SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 4);
	cout << RED << "\t| " << output << endl << RESET;
}
...
void printServices(const string service, const string serviceArray[], const int size) {
	// REMOVED FOR LIUNX SUPPORT SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 2);
	cout << GREEN << "Eligiable services for " << service << endl;
	for (int i = 0; i < size; i++) { cout << "\t\t" << serviceArray[i] << endl; }
	cout << "Please enter your selection:" << RESET;
}
...
const void printInput(string &userInput, const string inputOutput) {
	// REMOVED FOR LIUNX SUPPORT SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 6);
	cout << YELLOW << inputOutput << RESET;
...
const void initializeBank() {
	...
	// REMOVED FOR LIUNX SUPPORT SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
	// REMOVED FOR LIUNX SUPPORT system("PAUSE");
}
